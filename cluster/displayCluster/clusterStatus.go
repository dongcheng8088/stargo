...
func ClusterStat(clusterName string) {
	...
	fmt.Printf("clusterVerison = %s\n", module.GConfigInfo.ClusterInfo.Version)
	...
	for i := 0; i < len(module.GConfigInfo.FeServers); i++ {
		tmpID = fmt.Sprintf("%s:%d", module.GConfigInfo.FeServers[i].Host, module.GConfigInfo.FeServers[i].EditLogPort)
		tmpRole = "FE"
		tmpHost = module.GConfigInfo.FeServers[i].Host
		tmpPort = fmt.Sprintf("%d/%d", module.GConfigInfo.FeServers[i].EditLogPort, module.GConfigInfo.FeServers[i].QueryPort)
		tmpDataDir = module.GConfigInfo.FeServers[i].MetaDir
		tmpDeployDir = module.GConfigInfo.FeServers[i].DeployDir
	...
	for i := 0; i < len(module.GConfigInfo.BeServers); i++ {
		...
		tmpID = fmt.Sprintf("%s:%d", module.GConfigInfo.BeServers[i].Host, module.GConfigInfo.BeServers[i].BePort)
		tmpRole = "BE"
		tmpHost = module.GConfigInfo.BeServers[i].Host
		tmpPort = fmt.Sprintf("%d/%d", module.GConfigInfo.BeServers[i].BePort, module.GConfigInfo.BeServers[i].HeartbeatServicePort)
		tmpDataDir = module.GConfigInfo.BeServers[i].StorageDir
