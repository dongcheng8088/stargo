...
func ModifyClusterConfig() {
	...
	for i := 0; i < len(module.GConfigInfo.FeServers); i++ {
		...
		tmpFeQueryPort := module.GConfigInfo.FeServers[i].QueryPort
		...
		configMap = make(map[string]string)
		configMap["priority_networks"] = module.GConfigInfo.FeServers[i].PriorityNetworks
		configMap["http_port"] = strconv.Itoa(int(module.GConfigInfo.FeServers[i].HttpPort))
		configMap["rpc_port"] = strconv.Itoa(int(module.GConfigInfo.FeServers[i].RpcPort))
		configMap["edit_log_port"] = strconv.Itoa(int(module.GConfigInfo.FeServers[i].EditLogPort))
		configMap["query_port"] = strconv.Itoa(int(module.GConfigInfo.FeServers[i].QueryPort))
		...
		tmpTargetFeConfPath := module.GConfigInfo.FeServers[i].DeployDir + "/conf/fe.conf"
	...
	for i := 0; i < len(module.GConfigInfo.BeServers); i++ {
		...
		tmpBeHeartbeatServicePort := module.GConfigInfo.BeServers[i].HeartbeatServicePort
		...
		configMap = make(map[string]string)
		configMap["priority_networks"] = module.GConfigInfo.BeServers[i].PriorityNetworks
		configMap["be_port"] = strconv.Itoa(int(module.GConfigInfo.BeServers[i].BePort))
		configMap["webserver_port"] = strconv.Itoa(int(module.GConfigInfo.BeServers[i].WebServerPort))
		configMap["heartbeat_service_port"] = strconv.Itoa(int(module.GConfigInfo.BeServers[i].HeartbeatServicePort))
