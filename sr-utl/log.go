...
// LogConfig 日志配置，所有字段均支持热更新（FilePath 除外）
type LogConfig struct {
	...
	FilePath   string   // 日志文件路径（不支持热更新，修改需先关后开）
	MaxSize    int64    // 单个日志最大字节，超过轮转（支持热更新）
}
...
// 不支持热更 FilePath，如需修改请先关闭文件日志再重新开启
...
// rotateFileLocked 日志轮转（调用方需保证 fileMu 锁持有）
